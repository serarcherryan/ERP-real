...
| TC-004 | Integration | 机构过滤 | 后端 API 可用 | 使用不同认证上下文查询列表 | 不返回其他机构档案，接口不接受客户端 `facilityId` 覆盖 | 是 | P0 |
...
| TC-013 | Contract | 幂等创建 | 后端 API 接入 | 重复提交相同 Idempotency-Key | 只创建一条档案 | 是 | P0 |
...
| TC-020 | Security | 审计落库 | 后端审计接入 | 查看明文敏感字段、修改、作废；导出接口未实现 | 已实现的敏感查看、修改、作废生成审计日志；导出等待异步任务模块 | 部分 | P0 |
